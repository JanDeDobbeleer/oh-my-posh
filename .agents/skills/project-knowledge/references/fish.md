...
- `commandline --is-valid` exit codes (fish >= 3.4, verified on 4.1.2, 2026-09-11): 0 = valid,
  ...
  `--is-valid`.
- A strictly empty buffer also reports status 1 (`builtins/commandline.rs`:
  `if current_buffer.is_empty() { return Err(STATUS_CMD_ERROR); }`, checked before
  `detect_parse_errors` runs) - it is indistinguishable from a genuine parse error by exit code
  alone. Any handler that branches on `--is-valid` returning 1 to mean "erroneous" must also
  check the buffer is non-empty, or every plain Enter press on an empty prompt gets treated as
  an error (this broke `_omp_enter_key_handler` entirely in an earlier version - Enter on an
  empty line silently did nothing instead of drawing a new prompt).
