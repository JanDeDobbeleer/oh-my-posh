...
When adding a new segment, five artifacts are required - use the `segment-create` skill to scaffold all of them automatically:
...
5. Register the type in `src/config/segment_types.go` via `gob.Register(&segments.MySegment{})` - missing this causes silent failures at runtime
