...
Get-ChildItem -Path "../../themes" -Recurse -File -Filter "*.omp.*" | ForEach-Object {
    "`"../../themes/$($_.Name)`" `"/themes/$($_.Name)`"" >> $mapping
}

$makeappx = "C:/Program Files (x86)/Windows Kits/10/bin/$SDKVersion/x64/makeappx.exe"

& "$makeappx" pack /p $installerMSIX /v /o /m $manifest /f $mapping
...
Write-Host "Finished building MSI and MSIX packages for $Architecture"
