...
type store struct {
	...
	// purely in-memory for the lifetime of this process: it must not be
	// marked dirty and close() must not attempt to (re)create the file,
	// since doing so would truncate the other process's data.
	locked bool
}
