...
func (d *Daemon) RenderPrompt(req *ipc.PromptRequest, stream ipc.DaemonService_RenderPromptServer) error {
	...
	// Serialize render operations (terminal uses global state)
	d.renderMu.Lock()
	...
	// Create engine with per-request template cache and text cache
	eng := &prompt.Engine{
		Config:        cfg,
		Env:           env,
		Plain:         flags.Plain,
	...
	updateCallback := func(segmentName string) {
		// Re-render prompt with updated state
		newPrompt := eng.ReRender()

		// Cache the segment's text (Engine computes the cache key internally)
	...
	pendingCount = len(eng.PendingSegments())
	...
	if pendingCount > 0 {
		...
		case <-ctx.Done():
			return ctx.Err()
		case <-pendingDone:
			// All segments completed, now send final response
			finalPrompt := eng.ReRender()
			finalResponse := &ipc.PromptResponse{
				Type:      "complete",
				RequestId: req.RequestId,
				Prompts:   d.buildPrompts(eng, finalPrompt),
			}
