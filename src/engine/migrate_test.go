...
func TestSegmentTemplateMigration(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			Expected: " {{ if .SSHSession }}\udb80\udf18 {{ end }}{{ .UserName }} ",
			Type:     SESSION,
			Props: properties.Map{
				"display_host": false,
			},
		},
		{
			...
			Expected: " {{ if .SSHSession }}\udb80\udf18 {{ end }}{{ .HostName }} ",
			Type:     SESSION,
			Props: properties.Map{
				"display_user": false,
			},
		...
		},
		{
			...
			Expected: " {{ if .SSHSession }}\udb80\udf18 {{ end }}<#123456>{{ .UserName }}</>@<#789012>{{ .HostName }}</> ",
			Type:     SESSION,
			Props: properties.Map{
				"user_color": "#123456",
				"host_color": "#789012",
