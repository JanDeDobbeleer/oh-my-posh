...
func (r *regquery) getRegistryKeyValue(regPath string, regKey string) (bool, string) {
	...
	if regRootHKeyHandle == 0 {
		return false, fmt.Sprintf("Error, Supplied root HKEY value not valid: '%s'", regPathParts[0])
	}
	...
	if regPathUTF16ConversionErr != nil {
		return false, fmt.Sprintf("Error, Could not convert supplied path '%s' to UTF16, error: '%s'", regPathParts[1], regPathUTF16ConversionErr)
	}
	...
	if regKeyUTF16ConversionErr != nil {
		return false, fmt.Sprintf("Error, could not convert supplied key '%s' to UTF16, error: '%s'", regKey, regKeyUTF16ConversionErr)
	}
