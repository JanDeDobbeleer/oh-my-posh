...
func (g *Git) hasPoshGitStatus() bool {
	...
	if len(g.Upstream) != 0 && g.getFetchUpstreamIcon() {
...
func (g *Git) parsePoshGitHEAD(head string) string {
	...
	if strings.HasSuffix(head, "...)") {
		...
		return fmt.Sprintf("%s%s", g.getCommitIcon(), head)
	}
	// tag
	if strings.HasPrefix(head, "(") {
		head = strings.TrimLeft(head, "(")
		head = strings.TrimRight(head, ")")
		return fmt.Sprintf("%s%s", g.getTagIcon(), head)
	}
	// regular branch
	return fmt.Sprintf("%s%s", g.getBranchIcon(), g.formatBranch(head))
}
