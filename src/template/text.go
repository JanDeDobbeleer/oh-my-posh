	...
	for i, char := range t.template {
		...
		case ' ', '}', ')': // space or }
			...
			case strings.HasPrefix(prop, ".Segments") && !strings.HasSuffix(prop, ".Contains"):
				...
				if len(parts) > 3 {
					result.WriteString(fmt.Sprintf(`(.Segments.MustGet "%s").%s`, parts[2], strings.Join(parts[3:], ".")))
				} else {
					result.WriteString(fmt.Sprintf(`(.Segments.MustGet "%s")`, parts[2]))
				}
			...
			case strings.HasPrefix(prop, ".Env."):
				...
				result.WriteString(fmt.Sprintf(`(call .Getenv "%s")`, strings.TrimPrefix(prop, ".Env.")))
			default:
