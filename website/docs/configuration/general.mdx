...
| `daemon_timeout`            | `int`            | `100`   | timeout in milliseconds before returning partial results when using [daemon mode][daemon]. Lower values show the prompt faster but with more pending indicators                                                                                                               |
| `daemon_idle_timeout`       | `string`         | `"5"`   | how long (in minutes) to keep the daemon running when no terminal sessions are active. Set to `"none"` to disable auto-shutdown. See [daemon mode][daemon]                                                                                                                   |
| `render_pending_icon`       | `string`         | ``     | icon displayed for segments that are still loading when using [daemon mode][daemon]                                                                                                                                                                                          |
| `render_pending_background` | `string`         |         | background [color][colors] for segments in pending state when using [daemon mode][daemon]                                                                                                                                                                                     |
