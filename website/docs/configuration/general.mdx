...
| `terminal_features`         | `map[string][]string`        |         | override which terminals support a given feature. Keys are feature names (e.g. `progress`), values are lists of terminal program names matched against the `$TERM_PROGRAM` environment variable (or `Windows Terminal` when `$WT_SESSION` is set). Omitting a key keeps the built-in default (e.g. `progress` defaults to `Windows Terminal` only). |
