...
## Template Expressions in Options

Numeric and boolean segment options accept Go [template][templates] expressions in addition to their
static types. This lets you compute option values dynamically — for example, sizing a path segment
based on the terminal width.

### Example

Instead of duplicating segments for different terminal widths:
...
When the option value is a string containing `{{ }}`, it is resolved as a template before being
converted to the target type (integer, float, or boolean). If resolution fails, the default value
is used.

Available template functions include `add`, `sub`, `mul`, `div`, and all other
[sprig](http://masterminds.github.io/sprig/) functions. The template context is the segment
writer itself: any exported method or field on the writer is reachable as `{{ .FieldName }}`.
For example, the `path` segment exposes `.TerminalWidth` (the terminal column count, or `0`
when it cannot be determined). Other segments expose their own fields — see the segment's
documentation page for what is available. A field that does not exist on the writer resolves
to the zero value, in which case the option falls back to its default.

:::note
This applies to **segment options** (inside `[blocks.segments.options]`), not to top-level segment
settings like `min_width` or `max_width` on the segment itself.
:::
