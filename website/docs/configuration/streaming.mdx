...
Streaming is skipped entirely inside PowerShell Editor Services - the host behind the VS Code
PowerShell extension and Neovim's PSES client - because that host runs its own pipelines from
the idle hook streaming relies on. The prompt renders synchronously there.
