...
| `{{ cmd "who" }}`                                                  | Run an OS command and return trimmed output. Pass additional arguments after the command name (e.g. `{{ cmd "git" "log" "--oneline" "-1" }}`). Returns an error if the command fails.             |
