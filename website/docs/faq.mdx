...
## Windows Terminal: Powerline separator color doesn't match the adjacent segment

Windows Terminal's [Adjust indistinguishable colors][wt-adjust-colors] setting can brighten or darken a foreground or
background color when it judges the contrast too low. Because a powerline separator's color is derived from the
adjacent segment's background, this setting can adjust the separator without adjusting the segment's actual
background fill (or vice versa), causing a visible mismatch right at the seam between two segments — even though the
theme's colors and Oh My Posh's own output (verified via `oh-my-posh debug`) are correct.

Disable it by setting `"adjustIndistinguishableColors": "never"` in Windows Terminal's `settings.json`, or via
...
## Repaint the prompt on demand

Oh My Posh renders the prompt once per command. When something changes while the prompt is displayed,
for example a theme reload or an environment variable set by a script, you can force a re-render
by calling a shell-specific function:

- Fish: `omp_repaint_prompt`
- Zsh: `omp_repaint_prompt`. It's also registered as a zle widget, so it can be bound to a key:

  ```zsh
  bindkey '^B' omp_repaint_prompt
  ```

- PowerShell: `Invoke-PoshPromptRepaint`
...
[wt-adjust-colors]: https://learn.microsoft.com/en-us/windows/terminal/customize-settings/profile-appearance#adjust-indistinguishable-colors
