...
Adjust the Oh My Posh init line in `~/.yashrc` by adding the `--config` flag with the location of your configuration.

```bash
eval "$(oh-my-posh init yash --config ~/jandedobbeleer.omp.json)"
```
