...
Advanced version: The name of the topic is defined by a separate variable `$themeName`, the path to the topics is obtained through an environment variable `$POSH_THEMES_PATH`. This also checks and informs about errors of specifying a theme file.

```bash
themeName="jandedobbeleer"
# For Windows Git BASH
# themePath=$(cygpath $POSH_THEMES_PATH)
themePath="$POSH_THEMES_PATH"
themeFile=$themePath/$themeName.omp.json
if [ ! -f "$themeFile" ]; then
    echo -e "\033[0;41m Warning: \033[0;33m Theme file is missing, check the path and name in the settings."
    echo -e "\033[32mOh-My-POSH start with the \"standart\" theme."
    eval "$(oh-my-posh init bash)"
else
    echo -e "\033[32mOh-My-POSH start with the \"$themeName\" theme."
    eval "$(oh-my-posh init bash --config $themeFile)"
fi
```
...
```lua title="oh-my-posh.lua"
local themeName = "gmay1"
local themeFile = os.getenv("POSH_THEMES_PATH") .. "\\" .. themeName .. ".omp.json"
if not io.open(themeFile) then
    print("\27[0;41m Warning: \27[0;33m Theme file is missing, check the path and name in the settings.")
    print("\27[32mOh-My-POSH start with the \"standart\" theme.")
    load(io.popen(string.format('oh-my-posh init cmd')):read("*a"))()
else
--    print(string.format('\27[32mOh-My-POSH start with the "%s" theme.', themeName))
    load(io.popen(string.format('oh-my-posh init cmd --config %s', themeFile)):read("*a"))()
end
```
...
```powershell
$themeName = "froczh"
$themeFile = $Env:POSH_THEMES_PATH + "\" + $themeName + ".omp.json"
if (-not (Test-Path $themeFile)) {
    Write-Host " Warning: " -BackgroundColor Red -NoNewline
    Write-Host " Theme file is missing, check the path and name in the settings." -ForegroundColor DarkYellow
    Write-Host "Oh-My-POSH start with the `"standart`" theme" -ForegroundColor DarkGreen
    oh-my-posh init pwsh | Invoke-Expression
}
else {
#    Write-Host "Oh-My-POSH start with the `"$themeName`" theme" -ForegroundColor DarkGreen
