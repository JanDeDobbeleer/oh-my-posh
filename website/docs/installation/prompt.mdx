...
To enable Antigravity CLI integration with Oh My Posh, add the following to your Antigravity CLI settings:

```json title="~/.gemini/antigravity-cli/settings.json"
{
  "statusLine": {
    "type": "command",
    "command": "oh-my-posh antigravity"
  }
}
```

:::tip
You can use the `--config` flag to point to a tailored Oh My Posh config just for Antigravity CLI:

```bash
oh-my-posh antigravity --config /path/to/antigravity.omp.json
```

:::

For more information about the Antigravity segment properties and customization options, see the
[Antigravity segment documentation].
...
[Copilot CLI segment documentation].
...
Add the following snippet as the last line to `~/.yashrc`:
...
absolute path. A package manager that removes the previous version during an upgrade can leave an
