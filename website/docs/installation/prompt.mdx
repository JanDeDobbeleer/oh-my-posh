...
:::tip Daemon Mode
For faster prompt rendering, you can enable [daemon mode][daemon] by adding the `--daemon` flag to your init command.
This runs Oh My Posh as a background process that caches configuration and segment data. Supported for bash, zsh, fish, and PowerShell.
:::
