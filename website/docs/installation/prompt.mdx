...
Microsoft Defender has a similar problem that appears to reduce performance.
Depending on your system, this speeds up execution by about 4 times compared to the default init command.
Tested with PowerShell 7.4.2 with Microsoft Defender fully enabled and requires NO exceptions for oh-my-posh.exe.

```powershell
& ([ScriptBlock]::Create([System.String]::Join((oh-my-posh init pwsh --config "$env:POSH_THEMES_PATH\jandedobbeleer.omp.json" --print), "`n")))
```
:::
