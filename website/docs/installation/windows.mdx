...
Replace `--scope user` with `--scope machine` if you want to install it for all users on the machine.
...
Replace `--scope user` with `--scope machine` if you want to upgrade it for all users on the machine.
