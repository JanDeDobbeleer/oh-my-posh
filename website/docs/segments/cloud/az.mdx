...
| -------- | :------: | :-----------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `source` | `string` | `first_match` | <ul><<li>`first_match`: try the CLI config first, then the PowerShell module. The first to resolve is displayed</li><li>`reverse_match`: try the PowerShell module first, then the CLI config. The first to resolve is displayed</li><li>`cli`: fetch the information from the CLI config</li><li>`pwsh`: fetch the information from the PowerShell Module config</li></ul> |
