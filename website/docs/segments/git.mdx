...
| `fetch_upstream_url`  | `boolean`           | fetch and format remote upstream URL - defaults to `false`                                                                                                                                                                                                                                                                            |
| `fetch_upstream_icon` | `boolean`           | fetch upstream icon - defaults to `false`                                                                                                                                                                                                                                                                                             |
| `instead_of_url`      | `[]string`          | fetch remote URL from git call if URL from local config starts with any value from the list. Use this for any value git is configured to rewrite. The value `["*"]` will always call git for the remote URL. Only relevant if `fetch_upstream_url` or `fetch_upstream_icon` is true                                                   |
...
| `upstream_icons`    | `map[string]string` | map of text (key) and icon/text (value). Value will be displayed if remote URL contains the key                                                                              |
