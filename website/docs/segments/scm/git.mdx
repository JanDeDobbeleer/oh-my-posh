...
`.MainWorktree` is resolved lazily only when referenced. Its first use in a linked worktree runs
`git worktree list --porcelain -z`; the result is cached for the shell session and shared by the
repository's linked worktrees.
