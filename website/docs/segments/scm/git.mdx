...
As doing multiple git calls can slow down the prompt experience, we only fetch what your config displays.
Oh My Posh inspects every template in your config - segment, color and style templates, cross-segment
references, extra prompts, tooltips, fillers, palette values and the console title - and runs a probe only
when one of them references a field it populates. For example, `.Working` or `.Staging` in a template
enables the status fetch, `.UpstreamIcon` the upstream icon fetch, and so on.

When a template is too dynamic to analyze exactly - template syntax inside an option value
(`branch_template`, for example) - Oh My Posh conservatively scans the raw template and option texts for
field names instead, so anything your config names still gets fetched. The one shape it cannot see through
is a template that prints its whole context (`{{ . }}`) without naming any field.

| Name                  |        Type         | Default | Description                                                                                                                                                                                                                                                                                                                           |
| --------------------- | :-----------------: | :-----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `native_status`       |      `boolean`      | `false` | fetch the status information using the built-in engine instead of the git CLI (experimental, falls back to git automatically). Falls back to the git CLI for sparse/split index, reftables, and submodule repositories                                                                                                                |
...
| `.Upstream`       | `string`            | the upstream reference, e.g. `origin/main`; empty when no upstream is configured                                                 |
...
| `.MainWorktree`   | `string`            | the absolute path to the main worktree; empty unless the current repository is a linked worktree                                 |
| `.IsBare`         | `boolean`           | if in a bare repo or not, fetched when referenced in a template                                                                  |
...
| `.User`           | `User`              | the current configured user, fetched when referenced in a template                                                               |
