...
Oh My Posh inspects your templates (including cross-segment references, extra prompts and the console title)
and runs a probe only when a template references one of the fields it populates. For example, `.Working` or
`.Staging` in a template enables the status fetch, `.UpstreamIcon` the upstream icon fetch, and so on.

The `fetch_*` options below remain available as explicit overrides: setting one to `true` or `false` always
wins over what the templates imply. When a template is too dynamic to analyze, only the explicitly enabled
options fetch, matching the previous opt-in behavior.
