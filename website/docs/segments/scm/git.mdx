...
The `fetch_*` options below remain available as explicit overrides: setting one to `true` or `false` always
wins over what the templates imply. When a template is too dynamic to analyze, or the segment uses template
syntax inside its options (`branch_template`, for example), only the explicitly enabled options fetch,
matching the previous opt-in behavior.
