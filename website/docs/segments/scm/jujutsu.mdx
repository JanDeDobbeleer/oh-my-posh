...
Working-copy status is fetched when your templates reference one of its fields (`.Working`, `.ChangeID`,
...): fetching is driven entirely by what you render. The `fetch_status` option no longer exists and is
ignored silently in existing configs.
