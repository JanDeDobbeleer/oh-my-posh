...
As doing multiple [subversion][svn] calls can slow down the prompt experience, we only fetch what your config displays.
Oh My Posh inspects every template in your config - segment, color and style templates, cross-segment
references, extra prompts, tooltips, fillers, palette values and the console title - and runs the status
fetch only when one of them references a field it populates.
